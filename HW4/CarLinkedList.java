...
public class CarLinkedList {
    ...
    public Car remove(String VIN) {
        // Checks if the carList is empty
        if (isEmpty()) return null;

        // Variables:
        String node_element = null;

        Node previous = null;
        Node current = null;
        Node tail = null;

        // Assigns the object at the beginning of the list to current
        current = head;

        // Initiates a Car instance and gets the first element in the list
        Car current_node_element = head.getElement();

        // Assigns the previous value to current
        previous = current;

        // An additional way of checking for the VINs that are being passed at the beginning of the list

        String current_n = current.getElement().getVIN();
        String prev_n = previous.getElement().getVIN();

        // Gets the VIN of the current node
        node_element = current_node_element.getVIN();


        while( current != null ) {

            // checks if the node element is equal to the VIN that is being
            // ingested through the main function
            if (node_element.equals(VIN)) {

                // we need to check if the value we start with is the first value in the list
                // need to treat the first value a bit different than the rest of the values
                // passed in the main function
                if (current == head) {
                        // we need to define the next node that head is going to be assign to
                        Node next_node = head;
                        // re assigned the head to the next value
                        head = head.getNext();
                        // decrement the count of the loop iteration
                        size--;
                        // return the next node or the new head
                        return next_node.getElement();
                }

                else  {
                        // checks where the VIN is currently located in the list ( not the head or tail)
                ...
                // if the loop from above doesn't match the current item reference the previous node
                // to the current node
                previous = current;

                // checks where the loop is currently at
                String pre_n = previous.getElement().getVIN();

                // walks down the list and gets the next element/object of the list, ( not the node)
                // we need this value to assign it to the node_element that is use in the while loop
                current = current.getNext();
                // we also need to check if the current value (the value passed through the main function)
                // exists in the csv file.
                // if the VIN doesn't exits it cannot fetch the element,
                // so current will equal to null, if we set current to the tail which is null, will lead to
                // the while loop to exit

                if (current == null) {
                    current = tail;
                }
                // if the element exist, assign the vin to the node_element which will check if the VIN is equal to the
                // vin in the csv file.

                else {
                    node_element = current.getElement().getVIN();
                }
